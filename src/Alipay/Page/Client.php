...
namespace Marketingsdk\EasyMarketing\Alipay\Page;
...
use Marketingsdk\EasyMarketing\Alipay\Page\Models\Page;
use Marketingsdk\EasyMarketing\Alipay\Clue\Models\Clue;
...
class Client {
     ...
     */
    public function getList($bizToken, $principalTag, $pageNo, $pageSize){
        $_runtime = [
        ...
        while (Tea::allowRetry(@$_runtime["retry"], $_retryTimes, $_now)) {
            ...
            try {
                ...
                $systemParams = [
                    "method" => "alipay.data.dataservice.ad.promotepage.batchquery",
                ...
                if ($this->_kernel->isCertMode()) {
                    if ($this->_kernel->verify($respMap, $this->_kernel->extractAlipayPublicKey($this->_kernel->getAlipayCertSN($respMap)))) {
                        return Page::fromMap($this->_kernel->toRespModel($respMap));
                    }
                }
                else {
